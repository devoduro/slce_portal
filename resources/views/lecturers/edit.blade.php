                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('lecturers.update', $lecturer) }}" class="space-y-6" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <x-input id="staff_id" name="staff_id" type="text" label="Staff ID" :value="old('staff_id', $lecturer->staff_id)" />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="department_id" name="department_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md">
                                    ...
                                    @foreach($departments as $department)
                                        <option value="{{ $department->id }}" {{ old('department_id', $lecturer->department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                                    @endforeach
                                </select>
                                @error('department_id')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
