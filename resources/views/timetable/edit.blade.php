                    ...
                    <form method="POST" action="{{ route('timetable.update', $entry) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="course_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md" required>
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" {{ old('course_id', $entry->course_id) == $course->id ? 'selected' : '' }}>{{ $course->code }} - {{ $course->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                <x-input id="venue" name="venue" type="text" label="Venue" :value="old('venue', $entry->venue)" required />
                            </div>
                        </div>
