...
@else
    <div class="overflow-x-auto pb-4">
        <div class="grid gap-px bg-gray-200 border border-gray-300 rounded-lg overflow-hidden text-[11px] shadow-sm"
             style="grid-template-columns: 64px repeat(7, minmax(160px, 1fr)); grid-template-rows: 48px repeat({{ count($slotLabels) }}, 34px);">
            ...
            @foreach($entries as $entry)
                ...
                <div class="relative {{ $color['bg'] }} text-white rounded-lg m-0.5 px-2 py-1.5 shadow-md overflow-hidden group ring-1 ring-black/10"
                     style="grid-column: {{ $entry->grid_column }}; grid-row: {{ $entry->grid_row_start }} / {{ $entry->grid_row_end }}; z-index: 10;"
                     title="{{ $entry->course->title ?? '' }} — {{ $entry->classGroup->name ?? 'No class' }} — {{ $entry->lecturer->name ?? 'Unassigned lecturer' }} — {{ $entry->venue->name ?? 'No venue' }}">
                    @if($showActions)
                    ...
                    @endif
                    <p class="font-bold truncate leading-tight">{{ $entry->course->code ?? 'N/A' }}</p>
