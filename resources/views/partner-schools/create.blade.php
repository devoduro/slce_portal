        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('partner-schools.store') }}" class="space-y-6">
                        @csrf

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input id="name" name="name" type="text" label="School Name" :value="old('name')" required autofocus />
                            </div>
                            ...
                            <div>
                                ...
                                @enderror
                                <p class="mt-1 text-xs text-gray-400">Only students whose programme matches this category can select this school.</p>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                <x-input id="location" name="location" type="text" label="Location" :value="old('location')" placeholder="e.g. Osu, Accra" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                @foreach([100, 200, 300, 400] as $level)
                                    <x-input id="capacity_level_{{ $level }}" name="capacity_level_{{ $level }}" type="number" min="0" label="Level {{ $level }}" :value="old('capacity_level_' . $level, 0)" required />
                                @endforeach
                            </div>
                            <div class="mt-4 max-w-xs">
                                <x-input id="total_capacity" name="total_capacity" type="number" min="0" label="Total Capacity (optional)" :value="old('total_capacity')" helper="Informational overall figure (e.g. a physical building limit). Leave blank to default to the sum of the levels above." />
                            </div>
                        </div>
